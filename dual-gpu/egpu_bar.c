...
#define GPU_DEVICE  0x73bf      /* RX 6800 / 6800 XT / 6900 XT (Navi 21) */

/* Each RX 6800 gets a 512MB 64-bit prefetchable window:
...
static int __init egpu_bar_init(void)
{
	...
	while ((gpu = pci_get_device(GPU_VENDOR, GPU_DEVICE, gpu)) != NULL) {
		 ...
		 * doorbells. This is the key difference from the BAR0-only setup. */
		bar2 = &gpu->resource[2];
	...
	saved_count = i;
	if (saved_count == 0) {
		pr_err("egpu_bar: no RX 6800 (1002:73bf) found\n");
		return -ENODEV;
	}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Fix eGPU BAR0 + doorbell BAR2 for RX 6800 on T2 Mac Mini (multi-GPU)");
